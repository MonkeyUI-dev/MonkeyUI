...
Please deeply analyze the design drafts I provide and extract the core design system elements. To ensure accuracy, please execute the following specific algorithms and logic, and finally generate structured design tokens.
...
```json
{
  "styleName": "string - Name describing the overall visual style (e.g., Playful Organic Minimalism, Apple Modern, Cyberpunk)",
  ...
  "typography": {
    "fontFamily": "string - Primary font family stack (e.g., 'Quicksand, sans-serif')",
    "fontFamilyHeading": "string - Heading font family if different (e.g., 'DynaPuff, cursive')",
    ...
    "textStroke": "string - Text stroke properties if applicable (e.g., '2px currentColor')"
  },
  "spacing": {
  ...
  "borderRadius": {
    ...
    "large": "string - Large radius for cards (e.g., 50px)",
    "full": "string - Full/pill radius (e.g., 100px or 9999px)"
  },
  "shadows": {
    "level1": "string - Subtle shadow for hover states (e.g., 0 2px 8px rgba(0,0,0,0.06))",
    "level2": "string - Medium shadow for dropdowns/popovers (e.g., 0 10px 30px rgba(0,0,0,0.12))",
    "level3": "string - Strong shadow for modals (e.g., 0 20px 60px rgba(0,0,0,0.20))"
  },
  "visualEffects": {
    "hasGlassmorphism": "boolean - Whether glassmorphism is used",
    "hasGradients": "boolean - Whether gradients are present",
    "gradientStyle": "string - Description of gradient usage if applicable",
    "hasAnimations": "boolean - Whether animations are evident",
    "backdropBlur": "string - Blur amount if glassmorphism is used (e.g., 10px)"
  },
  "styleRules": [
    "string - Specific style rules derived from the design",
    "e.g., Display headings use ultra-bold bubble sans with 4-6% outer stroke",
    "e.g., Main cards use extreme rounded corners (50px) with subtle ghost shadows",
    "e.g., Color sampling confidence: primary=#FBC35D (high), secondary=#8CC9F1 (medium - gradient detected)",
    "e.g., Iconography: 2.5D isometric style with soft inner shadows and matte finish",
    "e.g., Letter spacing: +0.03em to +0.05em for fat display fonts to maintain breathing room"
  ]
}
```
...
For context, here's an example of a well-extracted design system:

**Swirlzy Brand Design System (V3.1 - Precision Update)**

- **Visual Style & Vibe**: Playful Organic Minimalism. Elements should look "inflated" and doughy, with extreme rounded corners and high-contrast, delicious color blocking.
- **Color Palette**: Background: #F2F4E9 (Milk White), Primary Accent: #FBC35D (Honey Yellow), Action/CTA: #2B5F4E (Forest Green), Secondary Surfaces: #8CC9F1 (Sky Blue) & #F69C5C (Apricot), Text: #1A1A1A (Primary), #5C5C5C (Secondary)
- **Typography**: Display (H1/H2): Ultra-bold Bubble Sans (e.g., DynaPuff Style) with solid stroke (4-6% of font-size) in the same color. Use "Outer Stroke" or "paint-order: stroke fill" to inflate letters without losing inner gaps. Letter-spacing: +0.03em to +0.05em. Body: Geometric Rounded Sans (e.g., Quicksand), 500-700 weight.
- **UI Geometry**: Corner Radius: 50px for main cards, 100px (Pill) for buttons. Shadows: Very subtle "Ghost Shadows" (0 10px 30px rgba(0,0,0,0.05)). Layout: Overlapping product photography with large, high-contrast color blocks.
- **Implementation Note**: Prioritize the "expanded stroke" on headers to achieve the signature liquid-smooth roundness. Ensure letters are thick and fleshy but clearly separated.
...
- Include detailed implementation notes in styleRules for complex visual effects
